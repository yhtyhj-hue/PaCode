...
| M5 | 工程评测套件（改 bug / 加测 / 小重构）一次成功率 | 基线 ≥ 0.5；gate + periodic simulated；live 经 env 或 cc-switch（BASELINE passRate=1 @ 2026-07-17） |
...
| M5 工程评测 | ✅ golden + simulated；**live once-success** 经 env/`cc-switch`（3/3，passRate 1） |
