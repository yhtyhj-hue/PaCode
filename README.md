...
- **10 上下文源 + 5 层压缩** — 10 源（含 Recent Results）；L1 Budget-Reduction（限速，非剪消息）→ Snip → Microcompact → Collapse → Auto-compact（L4–L5 已实现）
...
- **CLI / REPL / Ink TUI** — Plan（`/plan`）、paste chips、live task 面板、↑↓ 历史与 slash 导航、`-p` headless、`--tui`、cc-switch
...
命令行名仍是 `pacode`（包名为作用域 `@sallon/pacode`）。

未发布或要跟 Gitee `main` 时：

```bash
npm install -g https://gitee.com/sallon/pa-code.git
# 或指定分支：npm install -g gitee.com:sallon/pa-code#main
```
