...
- **28 核心工具** — Bash/Read/Edit/Write/Grep/Glob/Task/TodoWrite + TaskList/Get/Stop、Team、Coordinator、Skill/ToolSearch、Config、NotebookEdit、ScheduleCron、LSP(diagnostics)、AskUser、Web*、MCP* 等；文件工具限制在工作区内
- **9 上下文源 + 5 层压缩** — Budget → Snip → Microcompact → Collapse → Auto-compact（L4–L5 已实现）
- **文件化记忆** — 用户级 `.paude/memory/` + 项目级 `.paude/projects/{hash}/`
- **4 层扩展机制** — Hooks → Skills → Plugins（工具/agent 注册）→ MCP（stdio / SSE / HTTP；WebSocket deferred）
...
```bash
npm test              # gate tests（Vitest，1 periodic skip）
npm run test:coverage # 覆盖率报告（仅 src/，lines/statements ≥ 80%）
npm run eval:gate     # 确定性 eval 套件（含 M1–M4，< 2s）
npm run eval:periodic # LLM eval 套件（需 ANTHROPIC_API_KEY）
```

当前：以 `npm test` / `npm run eval:gate` 为准；覆盖率门禁 **≥ 80%**（`src/**`，不含 `.claude/worktrees`）。Phase H–K 见 [docs/ROADMAP.md](./docs/ROADMAP.md)。
