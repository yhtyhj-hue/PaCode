...
describe('eval:gate:m5-hard-engineering-tasks', () => {
  let workDir: string;
  ...
  it('defines three hard engineering tasks', () => {
    expect(M5_HARD_TASKS).toEqual(['multi-file-bug', 'fail-then-fix', 'cross-module']);
  });

  for (const taskId of M5_HARD_TASKS) {
